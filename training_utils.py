...
batch_to_torch = lambda b_in,b_target: (th.tensor(b_in, dtype=th.long),
...
def run_training_loop(
  ...
  train_x: List[str], train_y: List[float],
  validation_x: List[str], validation_y: List[float],
  model_id: str = "experiment"
):
  (validation_batch_input, validation_batch_target) = batch_to_torch(validation_x, validation_y)
  (validation_batch_input, validation_batch_target) = list_to_device((validation_batch_input, validation_batch_target), device)

  # print(validation_batch_target)
  ...
  for epoch in range(epochs):
      ...
      for i in t:
          ...
          (batch_input, batch_target) = batch_to_torch(batch_input, batch_target)
          (batch_input, batch_target) = list_to_device((batch_input, batch_target), device)
          
          prediction = th.flatten(model(batch_input))
          ...
          if i % 10 == 0:
              model.eval()
              validation_prediction = th.flatten(model(validation_batch_input))
